...
case "$1" in
    pr | submit) 
        ...
        universe_dir_pref=../typst-packages-universe/packages/preview/ose-pic
        universe_dir=../typst-packages-universe/packages/preview/ose-pic/"$VER"
        [[ -d ../typst-packages-universe ]] &&
        [[ -d "$universe_dir_pref" ]] &&
        # [[ ! -d "$universe_dir" ]] &&
        rsync --dry-run -auv ./src/master/ --exclude ose-pic.pdf "$universe_dir/" &&
        echo "Seems that we can do this!" &&
        echo '    ' rsync -auv ./src/master/ --exclude ose-pic.pdf "$universe_dir/"
